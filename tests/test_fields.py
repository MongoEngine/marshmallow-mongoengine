...
class TestFields(BaseTest):
    ...
    def test_DictField(self):
        ...
        data = {
            "int_1": 1,
            "nested_2": {"sub_int_1": 42, "sub_list_2": []},
            "list_3": ["a", "b", "c"],
        }
        doc = Doc(data=data)
        dump_data = DocSchema().dump(doc)
    ...
    def test_GenericReferenceField(self):
        ...
        load_data = DocSchema().load(
            {"generic": {"id": str(sub_doc_a.id), "_cls": sub_doc_a._class_name}}
        )
    ...
    def test_MapField(self):
        ...
        doc = Doc(
            map={"a": MappedDoc(field="A"), "b": MappedDoc(field="B")},
            str={"a": "aaa", "b": "bbbb"},
        ).save()
        dump_data = DocSchema().dump(doc)
        assert dump_data == {
